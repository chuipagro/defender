# Architecture — Defender Tower Defense

## Principes C++20 appliqués
- **Concepts** pour contraindre les templates (`Updatable`, `Renderable`)
- **`std::span`** pour passer des ranges sans copie
- **RAII** partout — pas de `new`/`delete` nus, uniquement `unique_ptr` / `shared_ptr`
- **Composition > héritage** — `Entity` est une base légère, les comportements sont dans des composants
- **`std::variant` + `std::visit`** pour le State Machine sans virtual pur
- **Séparation logique / rendu** — `defender_lib` ne dépend pas d'un display
